...
def bootstrap_ci(values, alpha: float = 0.05, iters: int = DEFAULT_ITERS,
    ...
    # sum()/n rather than mean(): the resample loop is the hot path at benchmark scale
    # (300+ queries x 10k draws x every metric x every mode), and mean() copies its input.
    draw = rng.choices
    boot = sorted(sum(draw(values, k=n)) / n for _ in range(iters))
