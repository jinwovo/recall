...
> works with a free local LLM (Ollama). Backend unit tests, frontend build, sidecar syntax
...
highlights the cited source. The answer below is generated by a free local LLM (Ollama):
...
| RAG with `[n]` citations + groundedness floor | Grounded generation, "I don't know" handling | groundedness, citation coverage |
...
| Async ingestion | Concurrency, idempotency, incremental indexing | docs/s, zero dup/loss |
...
| `GET` | `/api/ask?q=` | SSE stream: `sources`, `token`, `done` events; grounded answer with `[n]` citations |
...
(**Recall — Overview**): LLM tokens by model, semantic-cache hits, retrieval p95 by mode, and
ingestion throughput. Grafana at `localhost:3001`, Prometheus at `localhost:9090`.
...
- Post-hoc LLM-judge groundedness check
- MinIO raw-doc storage on the ingestion path; dead-letter + retry
- Enable the Testcontainers idempotency integration test in CI; eval regression gate
